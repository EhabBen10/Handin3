...
void bookletPrint(int startPage, int endPage)
{
    ...
    for (int bookletNummer = 1; bookletNummer <= totalArk; ++bookletNummer)
    {
        // Udskriver information om arket og siderne
        std::cout << "Booklet " << bookletNummer << " indeholder siderne ";

        // Beregner de fire sider på arket
        for (int i = 0; i < 4; ++i)
        {
            int currentPage = startPage + (bookletNummer - 1) * 4 + i;
